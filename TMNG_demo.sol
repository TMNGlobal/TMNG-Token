...
contract Token is ERC20 {
    using SafeMath for uint256;
    uint public nonce;
    uint256 public _createdDate;
    uint256 public _startReleaseTime;
    uint256 public _lastReleasedDate;
    bytes32 public _hashOfLastRelease;
    uint256 public _finalReleaseTime;
    ...
    function freezeAmount(address target, uint256 amount) private {
    ...
    function transferAndFreeze(address target, uint256 amount) private {
        transfer(target, amount);
        freezeAmount(target, amount);
        releaseableAddresses.push(target);
    }

    function getFreezedAmount(address target) public view returns (uint256) {
        return frozenAmounts[target];
    }

    constructor() public ERC20("TMN Global", "TMNG") {
